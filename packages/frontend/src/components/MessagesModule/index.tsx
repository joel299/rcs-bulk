...
export function MessagesModule() {
  ...
  const [previewVariation, setPreviewVariation] =
    useState<MessageVariation | null>(null);
  ...
  async function updateVariation(
    id: string,
    updates: Partial<MessageVariation>
  ) {
    const updated = await patch(
      `/api/campaigns/${activeCampaign!.id}/variations/${id}`,
      updates
    );
  ...
  async function uploadImage(variationId: string, file: File) {
    setUploading(true);
    setUploadError(null);
    ...
    try {
      const res = await fetch(
        `${base}/api/campaigns/${activeCampaign!.id}/variations/${variationId}/image`,
        { method: "POST", credentials: "include", body: formData }
      );
      if (!res.ok) {
        const err = await res.json().catch(() => ({}));
        throw new Error(
          err.error ?? err.message ?? `Upload failed: ${res.status}`
        );
      }
      const data = await res.json();
      if (data.variation) {
        setVariations(
          variations.map((v) => (v.id === variationId ? data.variation : v))
        );
        setPreviewVariation((prev) =>
          prev?.id === variationId ? data.variation : prev
        );
      } else if (data.imageUrl) {
        await updateVariation(variationId, { imageUrl: data.imageUrl });
      } else {
        throw new Error("Resposta inválida do servidor");
      }
      console.log(
        `[uploadImage] ✅ Image uploaded: ${data.imageUrl ?? data.variation?.imageUrl}`
      );
    } catch (err) {
      ...
      >
        ...
        >
          <div>
            ...
            >
              Use o botão <strong>{"{ }"}</strong> para inserir variável no
              cursor
            </p>
            {uploadError && (
              <p
...
function VariationCard({
  ...
  return (
    <GlassCard
      padding="16px"
      style={{
        outline: isPreview ? "1px solid var(--accent)" : "none",
        cursor: "pointer",
      }}
      onClick={onEdit}
    >
