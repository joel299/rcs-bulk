...
export function ScheduleModule() {
  const { activeCampaign, setActiveCampaign, removeCampaign } =
    useCampaignStore();
  const { post, patch, get, del } = useApi();
  ...
  useEffect(() => {
    if (
      !activeCampaign ||
      !["running", "waiting_window"].includes(activeCampaign.status)
    ) {
  ...
  const loadMoreLog = useCallback(async () => {
    ...
    setLogLoading(true);
    try {
      const params = logCursor
        ? `?before=${encodeURIComponent(logCursor)}&limit=20`
        : "?limit=20";
  ...
  useEffect(() => {
    ...
    const hasHistory = [
      ...
      "cancelled",
      "paused",
    ].includes(activeCampaign.status);
  ...
  useEffect(() => {
    ...
    const observer = new IntersectionObserver(
      (entries) => {
        if (entries[0].isIntersecting) loadMoreLog();
      },
      { threshold: 0.1 }
    );
  ...
  function startProgressStream(id: string) {
    ...
    es.onmessage = (e) => {
      const data: CampaignProgress & { recentDispatched?: LogEntry[] } =
        JSON.parse(e.data);
      setProgress(data);

      // Atualiza activeCampaign com contadores em tempo real
      if (activeCampaign) {
        setActiveCampaign({
          ...activeCampaign,
      ...
      const recent: LogEntry[] =
        data.recentDispatched ??
        (data.lastDispatched ? [data.lastDispatched as LogEntry] : []);
      if (recent.length > 0) {
        setLog((prev) => {
          const newEntries = recent.filter(
            (e) => !seenDispatched.current.has(e.dispatchedAt)
          );
  ...
  async function updateSchedule(updates: Partial<Campaign>) {
    const updated = await patch(
      `/api/campaigns/${activeCampaign!.id}`,
      updates
    );
  ...
  async function cancelCampaign() {
    if (!confirm("Cancelar a campanha? Esta ação não pode ser desfeita."))
      return;
    setActionLoading(true);
    setActionError(null);
    try {
  ...
  async function restartCampaign() {
    if (
      !confirm(
        "Reiniciar a campanha? Todos os contatos serão enviados novamente."
      )
    )
      return;
    setActionLoading(true);
    ...
    try {
      ...
      await post(`/api/campaigns/${id}/restart`, {});
      await post(`/api/campaigns/${id}/start`, {});
      const fresh = await get(`/api/campaigns/${id}`);
      setActiveCampaign(fresh);
      setProgress(null);
      setLog([]);
      seenDispatched.current = new Set();
      if (["running", "waiting_window"].includes(fresh.status)) {
  ...
  async function confirmDeleteCampaign() {
    ...
    setActionError(null);
    try {
      await del(`/api/campaigns/${activeCampaign.id}`);
  ...
  const isRunning = ["running", "waiting_window"].includes(
    activeCampaign.status
  );
  const canStart = ["draft", "paused", "scheduled", "waiting_window"].includes(
    activeCampaign.status
  );
  const canRestart = ["cancelled", "completed"].includes(activeCampaign.status);
  const canDeleteCampaign = ["completed", "cancelled", "draft"].includes(
    activeCampaign.status
  );
  ...
  const { label: statusLabel, color: statusColor } = statusBadge[
    activeCampaign.status
  ] ?? { label: activeCampaign.status, color: "gray" };
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
      ...
      {(isRunning ||
        ...
        <GlassCard>
          <ProgressBar
            value={
              (progress?.sentCount ?? activeCampaign.sentCount) +
              (progress?.failedCount ?? activeCampaign.failedCount)
            }
            max={progress?.totalContacts ?? activeCampaign.totalContacts}
            label="Progresso"
          />
          <div
            style={{ display: "flex", gap: 16, marginTop: 12, fontSize: 13 }}
          >
      ...
      <GlassCard>
        ...
        <div>
          ...
          >
            ...
            <strong>
              {activeCampaign.intervalMinSeconds}s –{" "}
              {activeCampaign.intervalMaxSeconds}s
            </strong>
          </p>
          <div style={{ display: "flex", gap: 12, alignItems: "center" }}>
            <span
              style={{ fontSize: 12, color: "var(--text-tertiary)", width: 32 }}
            >
              Mín
            </span>
            <input
              ...
              disabled={isRunning}
              onChange={(e) =>
                updateSchedule({ intervalMinSeconds: Number(e.target.value) })
              }
          ...
          >
            ...
            <input
              ...
              disabled={isRunning}
              onChange={(e) =>
                updateSchedule({ intervalMaxSeconds: Number(e.target.value) })
              }
              style={{ flex: 1, accentColor: "var(--accent)" }}
            />
            <span
        ...
        >
          ...
          {canDeleteCampaign && (
            <Button
              ...
              title={
                ["running", "waiting_window"].includes(activeCampaign.status)
                  ? "Pause ou cancele antes de excluir"
                  : "Excluir campanha"
              }
            >
              Excluir
      ...
      {deleteModalOpen && activeCampaign && (
        <div
          role="dialog"
          aria-modal="true"
        ...
        >
          <GlassCard padding="24px" style={{ maxWidth: 420, width: "100%" }}>
            <h3
            ...
            >
              Todos os contatos, variações de mensagem e histórico de disparo
              desta campanha serão removidos permanentemente. Esta ação não pode
              ser desfeita.
            </p>
            <div
      ...
      {showLog && (
        <GlassCard padding="0">
          ...
          >
            {log.map((entry, i) => (
              <LogRow
                key={`${entry.dispatchedAt}-${i}`}
                entry={entry}
                isLast={i === log.length - 1 && !logHasMore}
              />
            ))}
...
function LogRow({ entry, isLast }: { entry: LogEntry; isLast: boolean }) {
  ...
  return (
    ...
    >
      ...
      <span
        style={{ color: "var(--text-tertiary)", fontSize: 11, flexShrink: 0 }}
      >
        {entry.dispatchedAt
          ? new Date(entry.dispatchedAt).toLocaleTimeString("pt-BR")
          : ""}
      </span>
    </div>
  );
