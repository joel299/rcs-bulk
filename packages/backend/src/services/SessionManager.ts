...
import { randomDelay, dismissAllDialogs } from "../playwright/actions";
import {
  getStorageStatePath,
...
class SessionManager {
  ...
  private async saveStorageState(numberId: string, page: Page): Promise<void> {
    ...
    try {
      ...
      console.error(
        `[SessionManager] Failed to save session for ${numberId}:`,
        err
      );
    }
  }
  ...
  private async flushStorageState(
    numberId: string,
    page: Page | null
  ): Promise<void> {
    ...
    try {
      ...
      await page.context().storageState({ path: sessionPath });
      console.log(
        `[SessionManager] Flushed storageState before close: ${sessionPath}`
      );
    } catch (err) {
      console.warn(
        `[SessionManager] flushStorageState failed for ${numberId}:`,
        err
      );
    }
  }
  ...
  private async tryMinimizeBrowserWindow(page: Page): Promise<void> {
    ...
    try {
      ...
      console.warn(
        "[SessionManager] Could not minimize browser window (non-critical)"
      );
    }
  }
  ...
  private async clearLocalSession(numberId: string): Promise<void> {
    ...
    if (session?.storageKeepaliveInterval)
      clearInterval(session.storageKeepaliveInterval);
  ...
  private watchAuthStateVisible(
    ...
    const poll = setInterval(async () => {
      try {
        ...
        const authenticated =
          url.includes("messages.google.com") &&
          !url.includes("/authentication");

        if (authenticated) {
          clearInterval(poll);
          console.log(
            `[SessionManager] Authenticated for ${numberId} — keeping browser for dispatch`
          );
          ...
          const session: SessionState = {
            ...
            storageKeepaliveInterval: this.startStorageKeepalive(
              numberId,
              page
            ),
          };
        ...
        console.error(
          `[SessionManager] Auth poll error for ${numberId}:`,
          String(err)
        );
      }
    }, 2_000);
    ...
    return setInterval(
      async () => {
        try {
          ...
          const isAlive =
            (await page.locator(Selectors.conversationList).count()) > 0;

          if (!isAlive) {
            console.log(
              `[SessionManager] Session lost for number ${numberId}, marking disconnected`
            );
            await db.query(
    ...
    return setInterval(
      async () => {
        try {
          ...
          console.warn(
            `[SessionManager] Keepalive storage save failed for ${numberId}:`,
            err
          );
        }
      },
  ...
  async getPage(numberId: string): Promise<Page | null> {
    ...
    if (existing?.page) {
      ...
      if (!alive) {
        ...
        try {
          ...
          console.warn(
            `[SessionManager] Could not mark number disconnected:`,
            err
          );
        }
      } else {
    ...
    console.log(
      `[SessionManager] Restoring session for ${numberId} (headless=false)`
    );
    ...
    if (url.includes("/authentication")) {
      console.warn(
        `[SessionManager] Session expired for ${numberId} — QR code required`
      );
      if (fs.existsSync(storagePath)) {
        try {
          ...
          console.warn(
            `[SessionManager] Could not remove storageState file:`,
            e
          );
        }
      }
    ...
    this.sessions.set(numberId, session);
    console.log(
      `[SessionManager] Session restored and authenticated for ${numberId}`
    );
      ...
      console.log(
        `[SessionManager] Number ${id} (${phone}) will restore lazily on first dispatch.` +
          (fs.existsSync(stPath)
            ? " storageState found."
            : " No storageState — will use Chrome profile.")
      );
    }
  }
}
