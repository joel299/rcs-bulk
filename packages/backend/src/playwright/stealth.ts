...
export async function launchStealthContext(
  ...
  if (storageStatePath && !useStorageState) {
    console.warn(
      `[stealth] storageState path missing on disk, skipping: ${storageStatePath}`
    );
  }

  const context: BrowserContext = await (
    chromiumExtra as typeof chromium
  ).launchPersistentContext(sessionDir, {
    headless,
    slowMo,
    executablePath,
    args,
